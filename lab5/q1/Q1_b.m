...
%create digital fourth IIR Chebyshev-Type I filter
y = zeros(2, length(x)); %output y
Omega_y = [300/sampling_frequency, 800/sampling_frequency];
Rp = 1;

%designing IIR Chebyshev -Type I filter with order 4
[b,a] = cheby1(4, Rp, Omega_y, 'Bandpass');
[H,f] = freqz(b, a);
y_n = filter(b, a, x);
...
ylabel('Y(W)');
title('Y(W) of Chebyshev Type I filter with order 4');
...
%designing IIR Chebyshev-Type I filter with order 8
Omega_y_order_8 = [300/sampling_frequency, 1500/sampling_frequency];
[b,a] = cheby1(8, Rp, Omega_y_order_8, 'Bandpass');
[H,f] = freqz(b, a);
...
xlabel('Omega (W) as a function of pi ');
ylabel('Y(W)');
title('Y(W) of Chebyshev Type I filter with order 8');
