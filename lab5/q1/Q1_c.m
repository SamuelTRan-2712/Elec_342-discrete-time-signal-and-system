...
%create digital fourth IIR Elliptic filter
y = zeros(2, length(x)); %output y
Omega_y = [350/sampling_frequency, 700/sampling_frequency];
Rp = 1;
Rs = 60;

%designing IIR Elliptic filter with order 4
[b,a] = ellip(4, Rp, Rs, Omega_y, 'Bandpass');
[H,f] = freqz(b, a);
y_n = filter(b, a, x);
...
ylabel('Y(W)');
title('Y(W) of Elliptic filter with order 4');
...
%designing IIR Elliptic filter with order 8
Omega_y_order_8 = [500/sampling_frequency, 1000/sampling_frequency];
[b,a] = ellip(8, Rp, Rs, Omega_y_order_8, 'Bandpass');
[H,f] = freqz(b, a);
...
xlabel('Omega (W) as a function of pi ');
ylabel('Y(W)');
title('Y(W) of Elliptic filter with order 8');
